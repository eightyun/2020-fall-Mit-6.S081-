...
#define PA2INDEX(pa) (((uint64)pa)/PGSIZE) // reference count for each physical page to facilitate COW
...
kfree(void *pa)
{
  ...
  if (remain > 0)  // 只有最后1个reference被删除时需要真正释放这个物理页
    return;
...
// help 函数
void 
adjustref(uint64 pa , int num)
{
  if(pa >= PHYSTOP)
    panic("adjustref: pa to big");
